...
async def test_bm25_retrieve_returns_chunk_results() -> None:
    ...
    mock_result.fetchall = MagicMock(
        return_value=[
            (chunk1_id, filing_id, "Apple faces supply chain risks", 1, "Risk Factors", "AAPL", "10-K", "2024-09-30"),
            (chunk2_id, filing_id, "Revenue grew significantly last year", 2, "MD&A", "AAPL", "10-K", "2024-09-30"),
            (chunk3_id, filing_id, "The company designs hardware products", 3, "Business", "AAPL", "10-K", "2024-09-30"),
        ]
    )
...
async def test_bm25_scores_normalized_0_to_1() -> None:
    ...
    mock_result.fetchall = MagicMock(
        return_value=[
            (uuid.uuid4(), filing_id, "Apple reported strong revenue growth", 1, "MD&A", "AAPL", "10-K", "2024-09-30"),
            (uuid.uuid4(), filing_id, "The company faces regulatory challenges", 2, "Risk", "AAPL", "10-K", "2024-09-30"),
            (uuid.uuid4(), filing_id, "Revenue increased by twenty percent", 3, "MD&A", "AAPL", "10-K", "2024-09-30"),
        ]
    )
    mock_session.execute = AsyncMock(return_value=mock_result)
...
async def test_bm25_top_k_respected() -> None:
    ...
    filing_id = uuid.uuid4()
    mock_rows = [
        (uuid.uuid4(), filing_id, f"Chunk number {i} with some text", i, "Section", "AAPL", "10-K", "2024-09-30")
        for i in range(10)
    ]
    mock_result = MagicMock()
...
async def test_bm25_keyword_relevance() -> None:
    ...
    mock_result.fetchall = MagicMock(
        return_value=[
            (chunk1_id, filing_id, "The company designs innovative products", 1, "Business", "AAPL", "10-K", "2024-09-30"),
            (chunk2_id, filing_id, "Revenue grew by fifteen percent this quarter", 2, "MD&A", "AAPL", "10-K", "2024-09-30"),
            (chunk3_id, filing_id, "Supply chain risks remain significant", 3, "Risk", "AAPL", "10-K", "2024-09-30"),
        ]
    )
    mock_session.execute = AsyncMock(return_value=mock_result)
