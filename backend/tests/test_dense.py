...
def test_chunk_result_dataclass() -> None:
    ...
    assert result.filing_date == "2024-09-30"
...
async def test_dense_retriever_returns_list() -> None:
    ...
    mock_result.fetchall = MagicMock(
        return_value=[
            (chunk_id, filing_id, "Test text", 1, "Risk Factors", "AAPL", "10-K", "2024-09-30", 0.95),
            (uuid.uuid4(), filing_id, "Another chunk", 2, "Business", "AAPL", "10-K", "2024-09-30", 0.88),
        ]
    )
...
async def test_dense_retriever_top_k_respected() -> None:
    ...
    filing_id = uuid.uuid4()
    mock_rows = [
        (uuid.uuid4(), filing_id, f"Chunk {i}", i, "Section", "AAPL", "10-K", "2024-09-30", 0.9 - i * 0.05)
        for i in range(5)
    ]
    mock_result = MagicMock()
